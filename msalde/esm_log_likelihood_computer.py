...
class ESM2LogLikelihoodComputer(LogLikelihoodComputer):
    ...
    def compute_log_likelihoods(self, variants: list[Variant]
        ...
        # Get the log probabilities for the WT sequence
        self._base_model.eval()
        wt_data = [("WT", self._wt_sequence)]
        ...
        with torch.no_grad():
            logits = self._base_model(
                batch_tokens, repr_layers=[], return_contacts=False
            )["logits"]
            wt_log_probs = torch.log_softmax(logits, dim=-1)

        # Score each mutant using the WT log probabilities
        for variant in variants:
            ...
            if pos is None:
                ...
                mut_log_prob = wt_log_probs[
                    0, pos + 1,
                    self._alphabet.get_idx(mutant_residue)
                ].item()
                wt_log_prob = wt_log_probs[
                    0, pos + 1,
    ...
    def compute_log_likelihoods_wrong(self, variants: list[Variant]
                                ) -> list[Variant]:
        """ Compute log-likelihoods for a list of protein variants in batches. """
        self._base_model.eval()
